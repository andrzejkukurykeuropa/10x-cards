...
- Policz całkowitą liczbę faz (z nagłówków `## Phase N:`) i utwórz jeden wpis TaskCreate na fazę (pojawiają się one na pasku stanu użytkownika):
...
Rytuał zatwierdzania końca fazy (patrz "Verification Approach" poniżej) przygotowuje pliki z **zestawu dotkniętych plików**, który utrzymujesz w pamięci roboczej przez całą fazę. Ten zestaw jest kanonicznym wejściem do `git add` — nigdy nie wracaj do heurystyk `git status` dla decyzji o przygotowaniu.
...
- Zestaw zawsze zawiera `context/changes/<change-id>/plan.md`, ponieważ każda faza powoduje co najmniej jedną modyfikację w sekcji `## Progress`. Dodaj go przy wejściu do fazy, jeszcze zanim jakiekolwiek pola wyboru zostaną zmienione.
...
- Ta lista zastępuje wszelkie heurystyki z `git status`. Jeśli zestaw dotkniętych plików to `{a.md, b.md, plan.md}`, ale `git status --porcelain` również zgłasza `c.md` jako brudny, `c.md` jest niepowiązany — obsłuż go za pomocą monitu o brudną ścieżkę w rytuale, nigdy nie dołączaj go cicho do commita.
...
- **Uruchom rytuał zatwierdzania końca fazy**: Po przejściu wszystkich automatycznych sprawdzeń dla fazy, przejdź przez ten sekwencyjny rytuał, aby utworzyć jeden commit Conventional-Commits i zapisać krótki SHA zamykający z powrotem do każdego wiersza Progress zmienionego podczas fazy.
  ...
  1. **Bramka ręcznego potwierdzenia.** Poinformuj człowieka, że automatyczna weryfikacja przeszła i wymień elementy ręcznej weryfikacji z planu. Zatrzymaj się tutaj. Nie kontynuuj, dopóki człowiek nie potwierdzi, że testy ręczne zakończyły się sukcesem. Użyj tego formatu:
     ...
     **Ręczne podsumowanie międzyfazowe (tylko faza końcowa).** Przed wydrukowaniem komunikatu bramki, określ, czy bieżąca faza jest fazą końcową: przeskanuj sekcję `## Progress` w poszukiwaniu nagłówków `### Phase M:` i traktuj bieżącą fazę jako końcową, jeśli w kolejności dokumentu nie istnieje nagłówek z `M > N`. Jeśli bieżąca faza **nie jest** końcowa, komunikat bramki ma dokładnie powyższy format — bez podsumowania. Jeśli bieżąca faza **jest** końcowa, po bloku "Please perform the manual verification steps listed in the plan:", przeskanuj całą sekcję Progress w poszukiwaniu wierszy `- [ ]`, które znajdują się pod podsekcją `#### Manual` w dowolnej fazie **innej niż bieżąca**. Jeśli takie wiersze istnieją, dołącz następujący blok do komunikatu bramki (w kolejności dokumentu, jeden wiersz na linię, sformatowany jako `<phase>.<index> <title>` — usuń wszelkie prefiksy `- [ ]` i wszelkie końcowe sufiksy ` — <sha>`):
     ...
     Jeśli nie ma oczekujących ręcznych wierszy z wcześniejszych faz, całkowicie pomiń blok podsumowania. Bramka nadal wstrzymuje się na potwierdzenie przez człowieka; jest to informacyjne, a nie twarda blokada. Fazy pośrednie (dowolna faza, która nie jest końcową) zachowują oryginalny format bramki bez podsumowania.

  2. **Oblicz zestaw przygotowania.** Weź zestaw dotkniętych plików utrzymywany podczas fazy (patrz "Tracking files touched during a phase" powyżej) i połącz go z `{context/changes/<change-id>/plan.md}`. Plik planu jest zawsze przygotowywany, ponieważ każda faza powoduje co najmniej jedną modyfikację w sekcji `## Progress`.

  3. **Wykryj niepowiązane brudne ścieżki.** Uruchom `git status --porcelain` i przetnij z ścieżkami *poza* zestawem przygotowania. Jeśli zestaw brudnych, ale niedotkniętych plików nie jest pusty, przedstaw problematyczne ścieżki i zapytaj użytkownika:
  ...
  10. **Zaktualizuj `change.md`.** Ustaw `updated: <today>`; zachowaj `status: implementing` (idempotentne do ostatniej fazy). W ostatniej fazie ustaw `status: implemented` po zapisaniu SHA (patrz "After all phases" poniżej).
...
Nie dodawaj sufiksu SHA przy edycji pojedynczego kroku — SHA jest zapisywane z powrotem na końcu fazy przez rytuał commitu (patrz "Verification Approach" powyżej), a tylko SHA zamykającego commitu trafia do każdego wiersza, który został zmieniony podczas fazy. W trakcie fazy, ukończone wiersze mają `[x]` bez sufiksu SHA; jest to prawidłowy stan pośredni.

### Po każdej fazie

Gdy wszystkie elementy `- [ ]` wewnątrz `### Phase N:` są teraz `- [x]`:

1. Uruchom rytuał zatwierdzania końca fazy (patrz "Verification Approach" powyżej): ręczne potwierdzenie → przygotowanie → monit o brudną ścieżkę → commit → zapis SHA.
...
4. **Uruchom commit epilogu.** Commit ostatniej fazy nie może zawierać własnego SHA (jajko i kura), więc zapis SHA z powrotem do wierszy Progress ostatniej fazy plus zmiana statusu `change.md` pozostają brudne w drzewie roboczym po zakończeniu rytuału ostatniej fazy. Utwórz jeden zamykający commit, aby je zatwierdzić — w przeciwnym razie twarda odmowa `/10x-archive` (niezatwierdzone ścieżki w folderze zmiany) zablokuje. Kroki:
   ...
   3. Zaproponuj temat `chore(<change-id>): close out plan (epilogue)` z krótką treścią odnotowującą końcowy zapis SHA planu + `change.md` → implemented, plus linię `Refs:` z "Tracking issue/task references for commits", jeśli ma zastosowanie. Poproś użytkownika o zatwierdzenie jako proponowane / edycję tematu / całkowite zastąpienie (te same opcje co w rytuale fazy).
